...
class PoissonCycle:
    ...
    def norm(self, U: Array) -> Array:
        r = residual(self.F, U, self.h, self.def_field)
    ...
    def do_cycle(self, F: Array, U: Array, def_field: Array, level: int, h: float) -> Array:
        if level <= 0 or min(U.shape) <= 2:
            return smooth_weighted_jacobi(U, F, h, iters=16, def_field=def_field, omega=2.0 / 3.0)

        # Pre-smooth
        U = smooth_weighted_jacobi(U, F, h, iters=self.v1, def_field=def_field, omega=2.0 / 3.0)

        # Residual and restrict to coarse
        r = residual(F, U, h, def_field)
        ...
        # Post-smooth
        U = smooth_weighted_jacobi(U, F, h, iters=self.v2, def_field=def_field, omega=2.0 / 3.0)
        return U
